  ...
  <div>
    ...
    <div class="container">
      <div class="bg-secondary-50 shadow overflow-hidden sm:rounded-lg">
        ...
        >
          <v-btn class="text neutral" :to="`/authors/${author.id}`">
            &larr; Cancel
          </v-btn>
...
export default {
  async asyncData({ $axios, params }) {
    const author = await $axios.$get(`/authors/${params.id}`);
    return { author };
  },
  methods: {
    async save() {
      await this.$axios.put(`/authors/${this.author.id}`, this.author);
      this.$router.push(`/authors/${this.author.id}`);
    }
  }
};
</script>
